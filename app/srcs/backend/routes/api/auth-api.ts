...
const authApi: FastifyPluginAsync = async (fastify: any) => {
	...
	fastify.post('/otp_verify_register', async (req: any, res: any) => {
		try
		{
			...
			if (!temp)
				return res.status(500).send({ message: 'Error: No OTP requested for this email.' });

			if (temp.expires < Date.now())
			{
				delete otpTemp[email];
				delete dataUserRegister[email];
				return res.status(400).send({ code: "auth.ERR_OTPExpired", message: 'Error: OTP expired, need to make a new request for OTP' });
			}

			if (temp.attempts >= 5)
				return res.status(429).send({ code: "auth.ERR_OTPManyTry", message: 'Too many attempts. Try again later.' });
			...
			if (!userData)
				return res.status(500).send({ message: 'Error: No user data for this email' });
			...
			res.status(500).send({ message: 'Server Error: Verify OTP - register' });
		}
	});
