...
export function TournamentLoading({ leaderboard, load, playerID, local }: LoadingProps) {
    const { t } = useLang();
    if (load || !leaderboard)
...
interface LocalLoadingProps {
    matches?: MatchPlayersData[];
    load: boolean;
}

export function LocalTournamentLoading({ matches, load}: LocalLoadingProps) {
    const { t } = useLang();
    if (load || !matches)
        return (<LoadingScreen progress={{step: t("loading.step_start"), completed: null, total: 1}} />);
