...
const submitPassword = async (event: React.FormEvent<HTMLFormElement>, user: User | null) => {
  event.preventDefault();
  const form = event.currentTarget;
  const input = form.elements.namedItem("password") as HTMLInputElement;
  const value = input.value.trim();
  if (!value) return toast.error("Empty password!");
  if (value.length < 8) return toast.error("Password length must longer than 8.");

  try {
    const data = await apiFetchPrivate("update_password", { method: "POST", body: JSON.stringify({ new_password: value}) });
...
export function MenuProfile({ user } : {user : User | null }) {
  ...
  React.useEffect(() => {
    setPreviewName(user?.acc.username ?? "HI");
  }, [user]);
  ...
  return (  
    ...
    <div className="mb-10 inset-0 w-screen h-[50%] gap-2 bg-[#D6D8D2]/40 backdrop-blur-sm flex justify-start">
      ...
      <div className="flex flex-col gap-2 justify-center align-middle">
        ...
        {/*update password not available for google user*/}
      </div>
      ...
      {password &&
      (<form className="flex gap-1 items-center" onSubmit={(event) => submitPassword(event, user)}>
