...
export function ProfileP({ t, toasterPluz }: TranslationProps) { 
  ...
  React.useEffect(() => {
    ...
    {/* Content */}
    <div className="relative w-[100dvw] h-[100dvh] flex flex-col justify-center gap-5">
      ...
      <div className="w-screen h-[40%] md:h-50% shadow-md shadow-gray-400 gap-2 bg-[#DBE2E9]/60 backdrop-blur-md flex justify-start">
        {menu === "Profile" ? <MenuProfile user={user} /> : <MenuOption user={user} refetch={refetchData} />}
      </div>

      {/* Action Button - Back */}

        <button className="self-center w-10 md:w-20 aspect-square border-2 border-silver rounded-md overflow-hidden hover:scale-90 transition-transform" onClick={() => navigate("/")}>
