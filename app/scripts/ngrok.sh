...
ENV="$SCRIPTS_DIR/../srcs/frontend/.env"
...
if [ -z "$NGROK_SECRET" ] || [ -z "$FRONTEND_PORT" ]; then
...
if ! grep -q "authtoken:" ~/.config/ngrok/ngrok.yml 2>/dev/null; then
	...
	(cd "$SCRIPTS_DIR/../srcs/frontend" && npx ngrok config add-authtoken "$NGROK_SECRET")
else
...
if pgrep -x "ngrok" >/dev/null; then
	...
	(cd "$SCRIPTS_DIR/../srcs/frontend" && npx ngrok http https://localhost:$FRONTEND_PORT > /dev/null &)
	NGROK_PID=$!
	sleep 2
	NGROK_URL=$(curl -s http://127.0.0.1:4040/api/tunnels | grep -o '"public_url":"[^"]*' | head -n 1 | sed 's/"public_url":"//')
	echo "Ngrok running at $NGROK_URL (forwarding https://localhost:$FRONTEND_PORT)"
